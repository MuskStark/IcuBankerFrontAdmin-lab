...
export default defineComponent({
  ...
  setup () {
    ...
    const handleQuery = () => {
      loading.value = true;
      jobs.value = [];
      axios.get('/tasks/admin/query').then((response) => {
        loading.value = false;
    ...
    const handleModalOk = () => {
      ...
      axios.post('/tasks/admin/' + url, job.value).then((response) => {
    ...
    const handleResume = (record) => {
      axios.post('/tasks/admin/reschedule', record).then((response) => {
        modalLoading.value = false;
        const data = response.data;
        if (data.status) {
          modalVisible.value = false;
